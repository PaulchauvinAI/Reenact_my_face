...
from app.configs import paths_config
...
def align_face(
    ...
    c, x, y = compute_transform(
        filepath_or_image, predictor, detector=detector, scale=scale
    )
...
def crop_faces(IMAGE_SIZE, files, scale, center_sigma=0.0, xy_sigma=0.0, use_fa=False):
    ...
    for _, path in tqdm(files):
        c, x, y = compute_transform(
            path, predictor, detector=detector, scale=scale, fa=fa
        )
        cs.append(c)
        xs.append(x)
        ys.append(y)
